            ...
            <form method="POST" action="{{ route('skladchinas.update', $skladchina) }}" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    @if($skladchina->image_path)
                        <img src="{{ url('images/'.$skladchina->image_path) }}?w=600" alt="{{ $skladchina->name }}" class="mb-2 w-full h-40 object-cover rounded">
                    @endif
                    ...
                    @if($skladchina->images->count())
                        <div class="flex flex-wrap gap-2 mb-2">
                            @foreach($skladchina->images as $img)
                                <img src="{{ url('images/'.$img->path) }}?w=200" alt="" class="w-20 h-20 object-cover rounded">
                            @endforeach
                        </div>
                    @endif
