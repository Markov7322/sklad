...
<div class="bg-white rounded-2xl shadow hover:shadow-lg overflow-hidden flex flex-col">
    @if($skladchina->image_path)
        <div class="w-full h-48 overflow-hidden relative group">
            <img
                src="{{ url('images/'.$skladchina->image_path) }}?w=600"
                srcset="{{ url('images/'.$skladchina->image_path) }}?w=300 300w, {{ url('images/'.$skladchina->image_path) }}?w=600 600w"
                sizes="(max-width: 640px) 300px, 600px"
                alt="{{ $skladchina->name }}"
                class="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105">
            @if($skladchina->images->first())
                <img
                    src="{{ url('images/'.$skladchina->images->first()->path) }}?w=600"
                    srcset="{{ url('images/'.$skladchina->images->first()->path) }}?w=300 300w, {{ url('images/'.$skladchina->images->first()->path) }}?w=600 600w"
                    sizes="(max-width: 640px) 300px, 600px"
                    alt=""
